...
var status = '';
const DraftListWithLanguage = ({route, navigation}) => {
  ...
  useEffect(() => {
    ...
    console.log("draft", route?.params?.item?.id)

    IsInternetConnected()
      .then(() => {
       // dispatch(draftLanguageRequest({category_id: route?.params?.item?.id}));
        dispatch(draftRequest({category_id: route?.params?.item?.id}));
      })
      .catch(() => {
        ToastMessage('Network connection issue');
      });
              ...
              renderItem={({item, index}) => {
                return (
                  <TouchableOpacity
                    ...
                    <Text style={{textAlign: 'center', color: active == item.key  ? COLORS.secondarColor  :  COLORS.themeColor}}>{item?.title}</Text>
                  </TouchableOpacity>
                );
              }}
            />
...
export default DraftListWithLanguage;
